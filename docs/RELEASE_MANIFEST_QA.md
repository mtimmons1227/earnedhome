...
_As of June 29, 2026. This is the approval + go-live checklist for the next production release._
...
| A6 | **Buyer-estimate email copy** | The buyer email content + disclosures (ships **dormant** — see B/C). Review before any Resend config turns sending on. | [`CHANGE_SIGNOFF_LOG.md`](CHANGE_SIGNOFF_LOG.md) #8 · `src/lib/email.ts` |
...
6. **Buyer-estimate email (Resend):** before it sends in **any** environment — set `RESEND_API_KEY` + a **verified** `RESEND_FROM` domain, and get Richard's RESPA sign-off on the email copy (A6). Leave both unset to keep the email dormant.
...
- Buyer estimate email: the **email itself now ships dormant on QA** (see Section B / A6); the **resume ("magic") link** to reopen a saved estimate is still not built.
