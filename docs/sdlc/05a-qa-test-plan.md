...
| **FHA card "Estimated Funds" does not foot to the "Estimated Total"** (found June 25) | **Engine (workbook) — presentation** | FHA **Loan Fees includes the financed UFMIP** (~1.75% of base loan) but **cash-to-close correctly excludes it** (it's financed, not paid in cash). So the column appears off by the UFMIP. Example: $500k / $50k down → Loan Fees $21,002 + Prepaids $6,078 + Down $45,000 − Seller $5,000 = **$67,080**, but Estimated Total = **$59,118** (Δ **$7,962** = UFMIP on the $455k loan). Confirmed in `/api/quote` JSON: FHA `loanFees: 21002`, `cashToClose: 59118`. The cash-to-close math is **right**; only the *display* doesn't add up. Conventional cards foot correctly. | **Richard** — decide presentation: (a) remove financed UFMIP from "Loan Fees" so the column foots, (b) break it onto its own line labeled "financed (not cash)", or (c) footnote it. App can implement whichever he picks. Raise at 6/25. |
