...
export function PathfinderTool({ tenantId, loName, loNmls, companyName, applyUrl, loPhone, bookingUrl, agentId, agentName, agentFirm, shareToken }: Props) {
  ...
  const routeMsg =
    quote && cards.length === 0
      ? eligibility?.routeMessage ??
        "Let’s talk through your options to find the right program for you. Use “Connect with my mortgage advisor” below."
      : undefined;
      ...
      <div className="grid">
        <div className="panel">
          ...
          <select value={occupancy} onChange={(e) => setOccupancy(e.target.value as Occupancy | "")}>
            {OCCUPANCIES.map((o) => (
              <option key={o} value={o} disabled={veteran && o !== "Primary"}>{o}</option>
            ))}
          </select>
          ...
          <div className="checks">
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
                <label style={{ margin: 0, opacity: occupancy !== "Primary" ? 0.5 : 1 }}><input type="checkbox" checked={veteran}
                  disabled={occupancy !== "Primary"}
        ...
        <div id="results">
          ...
          {!loading && quote && (
            <div className="panel">
              ...
              {agentName && <div className="route">Your agent: {agentName}{agentFirm ? ` with ${agentFirm}` : ""}</div>}
              ...
              {!leadDone ? (
                <button className="leadbtn" onClick={() => setShowLeadModal(true)}
                  ...
                  <StepBadge label="2" />Connect with my mortgage advisor
                </button>
              ) : (
        ...
        {!quote ? (
          ...
          <>
            <div className="ctahint">
              Changed a number? Recalculate. To finish, use “Connect with my mortgage advisor” above.
            </div>
            <button onClick={getPayments} disabled={loading || !formValid}
